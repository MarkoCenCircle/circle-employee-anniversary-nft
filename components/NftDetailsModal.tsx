          ...
          >
            <div className="flex gap-8 flex-col md:flex-row">
              ...
              <div className="mt-2 mb-6 flex flex-col gap-6">
                <DialogTitle as="h3" className="text-4xl leading-10 break-words">{token.title}</DialogTitle>
                <div className='flex flex-col gap-4'>
                  <p className="text-gray-400 text-2xl break-words">
                    {token.description}
                  </p>
                  <p className="text-gray-400 break-words text-2xl">
                    NFT Contract: <Link className='underline' href={`https://polygonscan.com/address/${token.address}`} target='_blank'
                                   rel="noopener noreferrer">{token.address.substring(0, 6)}...{token.address.substring(36)}</Link>
                  </p>
                </div>
              </div>
            </div>
